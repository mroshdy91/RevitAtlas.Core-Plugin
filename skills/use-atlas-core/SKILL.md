...
This client preview requires a separately qualified Atlas runtime; it does not install or activate that runtime. Check `atlas_status` before execution. Discover live session identities; never reuse an identity from a previous task. Multiple Revit processes require an explicit session. Source, broker package and loaded engine versions are separate; an unavailable engine service requires its qualified binary, not a different tool spelling.
